package com.qdaily.frame.manager;
...
import com.qdaily.frame.mbus.aidl.BusProvider;

/**
 * Created by yuelinghui on 17/8/10.
 */

public class NetWorkStatusManager extends MManager {
    ...
    private boolean checkIsWifi(Context context) {
    ...
    private class ConnectivityBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (DBG) {
                Log.d(TAG,
                        "onReceive(): mNetworkInfo=" + mNetworkInfo + " mOtherNetworkInfo = "
                                + (mOtherNetworkInfo == null ? "[none]" : mOtherNetworkInfo + " noConn=" + noConnectivity) + " mState="
                                + mState.toString());
            }

            mIsWifi = checkIsWifi(mContext);

            BusProvider.getBus().getReceiver(EventNetChange.class).onNetChange(mNetworkInfo, mOtherNetworkInfo, mState);
        }
    }

    public interface EventNetChange {
        void onNetChange(NetworkInfo mNetworkInfo, NetworkInfo mOtherNetworkInfo, State mState);
    }
}

...
public class LocalDisplay {
    ...
    public static void init(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 14 && Build.VERSION.SDK_INT < 17)
            try {
                SCREEN_REAL_WIDTH_PIXELS = Math.min((Integer) Display.class.getMethod("getRawWidth").invoke(d),(Integer) Display.class.getMethod("getRawHeight").invoke(d));
                SCREEN_REAL_HEIGHT_PIXELS = Math.max((Integer) Display.class.getMethod("getRawWidth").invoke(d),(Integer) Display.class.getMethod("getRawHeight").invoke(d));
        ...
        if (Build.VERSION.SDK_INT >= 17)
            try {
                ...
                SCREEN_REAL_WIDTH_PIXELS = Math.min(realSize.x,realSize.y);
                SCREEN_REAL_HEIGHT_PIXELS = Math.max(realSize.x,realSize.y);
            } catch (Exception ignored) {
